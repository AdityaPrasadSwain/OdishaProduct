...
const ReviewVerify = ({ onEditStep, onSubmit }) => {
    ...
    const SectionHeader = ({ title, editIndex }) => (
        <div className="flex justify-between items-center mb-4">
            <h4 className="font-bold text-slate-400 uppercase tracking-[2px] text-[10px]">{title}</h4>
            <button
                onClick={() => onEditStep(editIndex)}
                className="text-indigo-400 text-xs font-bold hover:text-indigo-300 bg-indigo-500/10 px-4 py-1.5 rounded-full transition-all border border-indigo-500/20"
            >
                Edit
            </button>
        </div>
    ...
    const InfoRow = ({ label, value }) => (
        <div className="flex justify-between py-2.5 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors px-2 rounded-lg -mx-2">
            <span className="text-slate-500 font-medium text-sm">{label}</span>
            <span className="text-slate-100 font-semibold text-sm text-right">{value || '-'}</span>
        </div>
    );

    return (
        <div className="animate-fade-in-up">
            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-6 mb-8 flex gap-5 items-center">
                <span className="text-4xl filter drop-shadow-[0_0_8px_rgba(123,97,255,0.5)]">✨</span>
                <div>
                    <h5 className="font-bold text-indigo-100 text-lg">Final Review</h5>
                    <p className="text-sm text-slate-400">Your creation is almost ready. Review all details carefully before publishing to the "UdraKala" marketplace.</p>
                </div>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-6 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="flex justify-between py-2.5 border-b border-white/5">
                            <span className="text-slate-500 font-medium text-sm">Stock</span>
                            <span className={`px-2 py-0.5 rounded text-xs font-bold ${productData.stockQuantity > 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                        ...
                        <div className="flex justify-between py-2.5">
                            <span className="text-slate-500 font-medium text-sm">COD Available</span>
                ...
                <div className="space-y-6">
                    <div className="p-6 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm">
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                                {productData.imageUrls.map((url, i) => (
                                    <div key={i} className="relative group flex-shrink-0">
                                        <img src={url} alt={`preview-${i}`} className="w-20 h-20 object-cover rounded-xl border border-white/10 group-hover:border-indigo-500/50 transition-all" />
                                    </div>
                                ))}
                            </div>
                        ) : (
                        ...
                        {productData.reelUrl && (
                            <div className="mt-4 border-t border-white/5 pt-4">
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Product Reel</p>
                                <div className="w-24 aspect-[9/16] bg-slate-900 rounded-xl overflow-hidden shadow-2xl border border-white/10">
                    ...
                    <div className="p-6 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm">
                        ...
                        {productData.specifications && productData.specifications.length > 0 ? (
                            ...
                            <p className="text-sm text-slate-500 italic">No additional attributes</p>
                        )}
                    </div>
                    ...
                    <div className="p-6 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-sm">
                        ...
                        <div className="flex justify-between py-2.5 border-b border-white/5">
                            <span className="text-slate-500 font-medium text-sm">Returns</span>
            ...
            <div className="mt-12 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-100 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/20 blur-[80px] -mr-32 -mt-32"></div>
                <div className="z-10">
                    <h4 className="font-bold text-slate-900 text-xl">Ready to Publish?</h4>
                    <p className="text-slate-600 text-sm">The product will be live on UdraKala immediately after publishing.</p>
                </div>
                <button
                    onClick={handlePublish}
                    disabled={submitLoading}
                    className={`z-10 px-10 py-4 rounded-xl font-bold uppercase tracking-widest transition-all transform hover:scale-105 shadow-xl ${submitLoading ? 'bg-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-black w-full md:w-auto'}`}
                >
