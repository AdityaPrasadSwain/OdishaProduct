...
const GuideSection = ({ title, icon: Icon, children, isOpen, onToggle }) => {
    return (
        <motion.div
            ...
            className="border border-border dark:border-border rounded-xl overflow-hidden mb-4 bg-bg-surface dark:bg-bg-dark shadow-sm hover:shadow-md transition-shadow duration-300"
        >
            <button
                onClick={onToggle}
            ...
            >
                <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-lg font-semibold text-text-primary dark:text-text-secondary">{title}</h3>
                </div>
                {isOpen ? <ChevronDown className="text-text-secondary" /> : <ChevronRight className="text-text-secondary" />}
            </button>
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        <div className="p-5 pt-0 border-t border-border dark:border-border text-text-secondary dark:text-text-secondary leading-relaxed">
                            {children}
                        </div>
                    </motion.div>
                )}
...
const Step = ({ number, title, text }) => (
    <div className="flex gap-4 mb-4 last:mb-0">
        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary-light dark:bg-primary-hover/30 text-primary dark:text-primary flex items-center justify-center font-bold text-sm">
            {number}
        </div>
        <div>
            <h4 className="font-medium text-text-primary dark:text-text-onDark mb-1">{title}</h4>
            <p className="text-sm text-text-secondary dark:text-text-secondary">{text}</p>
        </div>
    </div>
);
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-10">
            ...
            <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-text-onDark p-10 shadow-xl"
            >
                <div className="relative z-10">
                    <h1 className="text-4xl font-bold mb-4">Seller Success Guide</h1>
                    <p className="text-primary text-lg max-w-2xl">
                ...
                <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 bg-bg-surface/10 rounded-full blur-3xl"></div>
                <div className="absolute bottom-0 left-10 w-40 h-40 bg-primary/30 rounded-full blur-2xl"></div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                    ...
                    >
                        ...
                        <div className="bg-blue-50 dark:bg-primary-hover/20 p-4 rounded-lg mb-4 border-l-4 border-primary">
                            <h5 className="font-semibold text-primary dark:text-primary mb-1">Image Guidelines</h5>
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Card className="!p-4 border border-border shadow-none bg-bg-page dark:bg-bg-dark">
                                <span className="text-xs font-bold text-status-warning uppercase tracking-wide">Step 1</span>
                            ...
                            <Card className="!p-4 border border-border shadow-none bg-bg-page dark:bg-bg-dark">
                                <span className="text-xs font-bold text-primary uppercase tracking-wide">Step 2</span>
                        ...
                        <div className="mt-4 space-y-3">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-text-secondary">Payments are processed every <strong>Wednesday</strong> for all orders delivered up to the previous Sunday.</p>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                <CheckCircle size={18} className="text-status-success mt-0.5" />
                                <div>
                                    ...
                                    <p className="text-xs text-text-secondary">Commission and shipping fees are deducted automatically. Check 'Pending Earnings' in the Wallet tab.</p>
                                </div>
                            </div>
                        </div>
                    </GuideSection>
                    ...
                    >
                        ...
                        <p className="text-sm text-text-secondary mb-4">You have 48 hours to approve or reject a return request. If you suspect fraud, reject the return with a clear reason and photo evidence if possible.</p>
                        <div className="text-sm p-3 bg-red-50 dark:text-status-error/20 text-status-error dark:text-status-error rounded border border-red-100 dark:border-red-900/30">
                ...
                <div className="space-y-6">
                    <Card className="!border-t-4 !border-t-primary">
                        ...
                        <button className="w-full py-2 px-4 bg-bg-band dark:bg-bg-dark hover:bg-bg-band dark:hover:bg-bg-dark rounded-lg text-sm font-medium transition-colors">
                            Contact Support
                        </button>
                    </Card>
                    ...
                    <Card>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="p-2 bg-green-100 dark:bg-green-900/30 rounded-full text-status-success">
                                <Shield size={20} />
                            </div>
                            <h3 className="font-semibold text-lg">Policies</h3>
                        </div>
                        <ul className="space-y-2 text-sm text-text-secondary dark:text-text-secondary">
                    ...
                    <div className="bg-primary-hover rounded-xl p-6 text-text-onDark text-center">
                        <h3 className="font-bold text-xl mb-2">Grow Faster! 🚀</h3>
                        <p className="text-primary text-sm mb-4">Top sellers upload at least 5 new products every week.</p>
                        <button className="bg-bg-surface text-primary px-4 py-2 rounded-lg font-bold text-sm hover:bg-indigo-50 transition-colors shadow">
                            Add Product Now
                        </button>
                    </div>
                </div>
