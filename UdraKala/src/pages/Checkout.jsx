...
const Checkout = () => {
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            // 2. Verify Payment for ALL created orders (Mock Online Flow)
            ...
            if (error.errorCode === 'INSUFFICIENT_STOCK' || error.errorCode === 'OUT_OF_STOCK') {
                Swal.fire({
                    icon: 'warning',
                    title: 'Stock Issue',
                    text: error.message,
    ...
    return (
        <div className="min-h-screen bg-bg-page dark:bg-dark py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
            <Motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="max-w-6xl mx-auto"
            >
                <h1 className="text-4xl font-bold text-text-primary dark:text-text-onDark mb-8 text-center font-serif tracking-tight">Checkout</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-bg-surface dark:bg-bg-dark p-6 rounded-2xl shadow-sm border border-border dark:border-border">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-text-primary dark:text-text-onDark flex items-center gap-2">
                                    <MapPin size={22} className="text-primary" /> Delivery Address
                                </h2>
                                <button
                                    onClick={() => setIsAddressModalOpen(true)}
                                    className="text-sm text-primary hover:text-primary font-bold flex items-center gap-1 uppercase tracking-wider"
                                >
                            ...
                            {isAddressLoading ? (
                                ...
                                <div className="text-center p-8 border-2 border-dashed border-border dark:border-border rounded-2xl bg-bg-page dark:bg-bg-dark/50">
                                    ...
                                    {addresses.map((addr) => (
                                        <div
                                            ...
                                            className={`p-5 rounded-xl border-2 cursor-pointer transition-all relative ${selectedAddressId === addr.id
                                                ? 'border-primary bg-bg-band dark:bg-primary-hover/10 shadow-md'
                                                : 'border-border dark:border-border hover:border-border dark:hover:border-border'
                                                }`}
                                        >
                                            <div className="flex items-start gap-4">
                                                <div className={`mt-1 w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${selectedAddressId === addr.id ? 'border-primary' : 'border-border'
                                                    }`}>
                                                ...
                                                <div>
                                                    <p className="text-base text-text-primary dark:text-text-onDark font-bold">
                                                        {addr.street}
                                                    </p>
                                                    <p className="text-sm text-text-secondary dark:text-text-secondary mt-1 leading-relaxed">
                                                        {addr.city}, {addr.state} - {addr.zipCode}
                                                    </p>
                                                    <p className="text-xs text-text-secondary mt-2 uppercase font-bold tracking-wide">{addr.country}</p>
                                                    {addr.default && (
                                                        <span className="absolute top-4 right-4 text-[10px] bg-primary-light dark:bg-primary-hover/40 text-primary dark:text-primary px-2 py-0.5 rounded-full flex items-center gap-1 font-bold uppercase tracking-wider">
                        ...
                        <div className="bg-bg-surface dark:bg-bg-dark p-6 rounded-2xl shadow-sm border border-border dark:border-border">
                            <h2 className="text-xl font-bold mb-6 text-text-primary dark:text-text-onDark flex items-center gap-2">
                                ...
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`p-4 border-2 rounded-xl cursor-pointer transition-all flex flex-col items-center gap-3 active:scale-95 ${paymentMethod === method.id
                                            ? 'border-primary bg-bg-band dark:bg-primary-hover/10 text-primary dark:text-primary font-bold shadow-sm'
                                            : 'border-border dark:border-border hover:border-border dark:hover:border-border text-text-secondary'}`}
                                    >
                                        <method.icon size={24} className={paymentMethod === method.id ? 'text-primary' : 'text-text-secondary'} />
                                        <span className="text-sm">{method.label}</span>
                                    </button>
                                ))}
                            </div>

                            <div className="mt-4 p-4 bg-bg-page dark:bg-bg-dark/50 rounded-xl border border-border dark:border-border">
                                {paymentMethod === 'COD' ? (
                                    <div className="text-sm text-text-secondary dark:text-text-secondary flex items-start gap-2">
                    ...
                    <div className="bg-bg-surface dark:bg-bg-dark p-6 rounded-2xl shadow-sm border border-border dark:border-border h-fit sticky top-24">
                        <h2 className="text-xl font-bold mb-6 text-text-primary dark:text-text-onDark flex items-center gap-2">
                            <CheckCircle size={22} className="text-primary" /> Order Summary
                        </h2>
                        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                            {cart.map((item) => (
                                <div key={item.id} className="flex gap-4 items-center border-b border-border dark:border-border pb-4 last:border-0">
                                    <div className="w-16 h-16 rounded-lg bg-bg-band dark:bg-bg-dark overflow-hidden shrink-0 border border-border dark:border-border">
                                        <img src={item.images?.[0]?.imagePath || item.imagePath || '/placeholder.png'} alt={item.name} className="w-full h-full object-cover" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="text-sm font-bold text-text-primary dark:text-text-onDark truncate">{item.name}</h3>
                                        <p className="text-xs text-text-secondary dark:text-text-secondary mt-1">Quantity: {item.quantity}</p>
                                    </div>
                                    <p className="font-bold text-primary dark:text-primary whitespace-nowrap">₹{(item.price * item.quantity).toLocaleString()}</p>
                                </div>
                            ))}
                        </div>
                        <div className="mt-6 pt-6 border-t border-border dark:border-border flex justify-between items-center">
                            <span className="text-lg font-medium text-text-primary dark:text-text-onDark">Total Amount</span>
                            <span className="text-2xl font-bold text-primary">₹{totalAmount.toLocaleString()}</span>
                        </div>
                        {discount > 0 && (
                            <div className="flex justify-between items-center mt-2 text-status-success">
