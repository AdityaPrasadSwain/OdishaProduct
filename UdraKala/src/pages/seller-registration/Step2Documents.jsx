...
const FileUploadField = ({
    ...
    <div className="bg-bg-page p-4 rounded-lg border border-border">
        ...
        <input
            ...
            maxLength={maxLength}
            onChange={onChange}
            className={`w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-primary outline-none mb-3 ${error ? 'border-red-500' : 'border-border'}`}
            placeholder={placeholder}
        />
        {error && <p className="text-status-error text-xs -mt-2 mb-2">{error}</p>}

        <div className={`border-2 border-dashed rounded-lg p-4 flex flex-col items-center justify-center transition-colors ${fileError ? 'border-red-400 bg-red-50' : 'border-border hover:border-primary bg-bg-surface'}`}>
            {fileState ? (
                <div className="w-full flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-sm overflow-hidden max-w-[150px]">
                            <p className="font-medium text-text-secondary truncate">{fileState.name}</p>
                            <p className="text-xs text-text-secondary">{(fileState.size / 1024 / 1024).toFixed(2)} MB</p>
                        </div>
                    </div>
                    <button type="button" onClick={onRemove} className="text-status-error hover:text-status-error p-1.5 rounded-full">
                        <X size={18} />
                    </button>
                </div>
            ) : (
                <label className="cursor-pointer flex flex-col items-center w-full">
                    ...
                    <span className="text-sm text-text-secondary">Click to upload document</span>
        ...
        {fileError && <p className="text-status-error text-xs mt-1 text-center">{fileError}</p>}
    </div>
);
...
const Step2Documents = ({ onNext }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <h2 className="text-2xl font-bold mb-2 text-center text-text-primary">Document Verification</h2>
            <p className="text-center text-text-secondary text-sm mb-6">Please provide your legal business documents.</p>
            ...
            <button type="submit" disabled={loading} className="w-full py-3 mt-6 bg-gradient-to-r from-orange-500 to-orange-600 text-text-onDark rounded-lg font-bold shadow-md hover:from-orange-600 hover:to-orange-700 transition disabled:opacity-70 disabled:cursor-not-allowed">
