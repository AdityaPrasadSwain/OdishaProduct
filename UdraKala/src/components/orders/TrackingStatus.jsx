...
const TrackingStatus = ({ awb, currentStatus }) => {
    ...
    if (!awb) return <span className="text-text-secondary text-sm">Not Shipped</span>;
    ...
    return (
        <div className="mt-2">
            <div className="flex items-center gap-2">
                <span className={`px-3 py-1 rounded-full text-xs font-medium 
                    ${currentStatus === 'DELIVERED' ? 'bg-green-100 text-green-800' :
                        currentStatus === 'SHIPPED' ? 'bg-primary-light text-primary' :
            ...
            {isOpen && (
                <div className="mt-3 p-4 bg-bg-page rounded-lg border border-border text-sm">
                                ...
                                <div className="mt-2 pl-2 border-l-2 border-primary">
                                    {trackingData.scans.slice(0, 3).map((scan, idx) => (
                                        <div key={idx} className="mb-2">
                                            <p className="text-xs text-text-secondary">{scan.date} {scan.time}</p>
                                            <p className="text-text-secondary">{scan.activity} - {scan.location}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
